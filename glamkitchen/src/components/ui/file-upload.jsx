...
export const FileUploaderItem = forwardRef(({ index, className, children, ...props }, ref) => {
...
export const FileInput = forwardRef(({ className, children, ...props }, ref) => {
    ...
    return (
        <div ref={ref} {...props} className={`relative w-full ${isLOF ? "opacity-50 cursor-not-allowed " : "cursor-pointer "}`}>
            <div
                className={cn(
                    `w-full rounded-lg duration-300 ease-in-out ${dropzoneState.isDragAccept ? "border-green-500" :
                        dropzoneState.isDragReject || isFileTooBig ? "border-red-500" : "border-muted-foreground"}`
                )}
            >
                <Input
                    className="absolute w-full opacity-0 z-10 inset-0 h-full cursor-pointer"
                    {...rootProps}
                />
                {children}
            </div>
